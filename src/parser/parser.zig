...
pub const Parser = struct {
    ...
    fn parseDeclaration(self: *Parser) ParseError!Declaration {
        ...
        } else if (self.check(.test_keyword)) blk: {
            if (is_public) {
...
test "parse trait method signature details" {
    const source =
        \\trait Show {
        \\    fn show(self: Self) -> string
        \\}
...
test "parse trait with effect method" {
    const source =
        \\trait Logger {
        \\    effect fn log(self: Self, msg: string) -> string
        \\}
    ;
    var lex = lexer.Lexer.init(source);
    ...
    const decl = try parser.parseDeclaration();
    try std.testing.expect(decl.kind == .trait_decl);
    const method = decl.kind.trait_decl.methods[0];
    try std.testing.expect(method.is_effect);
    try std.testing.expectEqualStrings("log", method.name);
    try std.testing.expectEqual(@as(usize, 2), method.parameters.len);
}
