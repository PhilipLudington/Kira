...
- [x] Implement AST-to-IR lowering for control flow — convert if/else and match statements to IR basic blocks with branches. Update `src/ir/lower.zig`. Tests cover: lower if/else to branch, lower match to jump table, nested control flow. (completed 2026-03-05)
...
- [x] All implemented AST node types lower to IR
...
### Tasks
- [x] Design and implement package manifest format — extended existing `kira.toml` with version, description, license, authors, and [dependencies] section with version constraints (^, ~, >=, >, <=, <, =). Added validation for required fields (name, version) and semver format checking. Updated `src/config/toml.zig` and `src/config/project.zig`. (per DESIGN.md section "Module System") Tests cover: parse valid manifest, reject missing required fields, parse dependencies with version constraints, parse empty dependencies, semver validation. (completed 2026-03-05)
